...
action_0,
 ...
 action_18,
 action_19,
 action_20 :: () => Prelude.Int -> ({-HappyReduction (Either String) = -}
	   Prelude.Int 
	-> (Token)
...
happyReduce_1,
 ...
 happyReduce_6,
 happyReduce_7,
 happyReduce_8 :: () => ({-HappyReduction (Either String) = -}
	   Prelude.Int 
	-> (Token)
	-> HappyState (Token) (HappyStk HappyAbsSyn -> [(Token)] -> (Either String) HappyAbsSyn)
	-> [HappyState (Token) (HappyStk HappyAbsSyn -> [(Token)] -> (Either String) HappyAbsSyn)] 
...
happyExpList = Happy_Data_Array.listArray (0,47) ([5312,5312,256,512,0,0,0,5312,5312,256,1216,8448,2304,1216,0,0,5312,512,16640,5312,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser","Expr","Term","Factor","ID","NUM","'+'","'*'","'('","')'","'if'","'then'","'else'","%eof"]
...
happyReduction_6 (HappyTerminal (Token _ (TokenID happy_var_1)))
	 =  HappyAbsSyn4
		 (Var happy_var_1
	)
...
happyReduction_7 (HappyTerminal (Token _ (TokenNum happy_var_1)))
	 =  HappyAbsSyn4
		 (Num happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	Token _ (TokenID happy_dollar_dollar) -> cont 7;
	Token _ (TokenNum happy_dollar_dollar) -> cont 8;
	Token _ TokenPlus -> cont 9;
	Token _ TokenTimes -> cont 10;
	Token _ TokenLParen -> cont 11;
	Token _ TokenRParen -> cont 12;
	Token _ TokenIf -> cont 13;
	Token _ TokenThen -> cont 14;
	Token _ TokenElse -> cont 15;
	_ -> happyError' ((tk:tks), [])
	}
